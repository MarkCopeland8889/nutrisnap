...
const GEMINI_API_KEY = "AIzaSyDjE6Auy8b1C89Xpb9iuIlenDhsfjbxuME"; // Your Gemini API Key

export default function DashboardPage() {
  const router = useRouter();
  const [currentUser, setCurrentUser] = useState(null);
  const [userData, setUserData] = useState(null); // To store user's profile and goals
  const [isLoadingPage, setIsLoadingPage] = useState(true);
  const [todaysIntake, setTodaysIntake] = useState({ calories: 0, protein: 0, carbs: 0, fat: 0 });
  const [error, setError] = useState('');
  const [foodEntries, setFoodEntries] = useState([]); // To store and display logged food items

  const fetchAndSetTodaysIntake = useCallback(async (userId) => {
    if (!userId) return;
    // console.log("Fetching today's intake for user:", userId);
    try {
      ...
      // console.log("Today's intake updated:", { calories: newCalories, protein: newProtein, carbs: newCarbs, fat: newFat });
    } catch (fetchError) {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        try {
          ...
          // Consider redirecting to login or an error page if profile is essential and fails to load
        }
      } else {
  ...
  const handleFoodAnalysisComplete = async (analysisData, textInput, imageUrl) => {
    ...
    try {
      ...
      // alert("Meal logged successfully!"); // Using console log for less intrusive feedback
  ...
  const handleEditMeal = (mealId) => {
    alert(`Edit functionality for meal ID: ${mealId} is coming soon!`);
    console.log("Attempting to edit meal:", mealId);
    // Example navigation for a dedicated edit page:
  ...
  const handleDeleteMeal = async (mealId) => {
    ...
    if (window.confirm("Are you sure you want to delete this meal log?")) {
      try {
        ...
        // alert("Meal deleted successfully!"); // Less intrusive feedback
        console.log("Meal deleted successfully!");
        await fetchAndSetTodaysIntake(currentUser.uid); // Refresh list and totals
      } catch (deleteError) {
  ...
  return (
    <>
      <Head>
        <title>Your Dashboard - NutriSnap</title>
      </Head>
          ...
          <div className="mb-8 p-6 bg-white rounded-xl shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-center mb-6">
              <div className="p-4 bg-green-50 rounded-lg shadow">
                <h3 className="text-sm font-medium text-green-700">Calories Remaining</h3>
                <p className="text-2xl font-bold text-green-800">
                  {remainingCalories.toLocaleString()} / {goals.dailyCalorieGoal?.toLocaleString() || 0}
                </p>
                <p className="text-xs text-slate-500">kcal ({todaysIntake.calories.toLocaleString()} eaten)</p>
              </div>
              <div className="p-4 bg-sky-50 rounded-lg shadow">
                <h3 className="text-sm font-medium text-sky-700">Protein</h3>
                <p className="text-2xl font-bold text-sky-800">{remainingProtein}g / {goals.dailyProteinGoal || 0}g</p>
                 <p className="text-xs text-slate-500">({todaysIntake.protein}g eaten)</p>
              </div>
              <div className="p-4 bg-amber-50 rounded-lg shadow">
                <h3 className="text-sm font-medium text-amber-700">Carbs</h3>
                <p className="text-2xl font-bold text-amber-800">{remainingCarbs}g / {goals.dailyCarbGoal || 0}g</p>
                <p className="text-xs text-slate-500">({todaysIntake.carbs}g eaten)</p>
              </div>
              <div className="p-4 bg-rose-50 rounded-lg shadow">
                <h3 className="text-sm font-medium text-rose-700">Fat</h3>
                <p className="text-2xl font-bold text-rose-800">{remainingFat}g / {goals.dailyFatGoal || 0}g</p>
                <p className="text-xs text-slate-500">({todaysIntake.fat}g eaten)</p>
              </div>
            </div>
          </div>
                ...
                {foodEntries.map(entry => (
                  <li key={entry.id} className="p-3 border border-slate-200 rounded-lg bg-slate-50 shadow-sm flex flex-col sm:flex-row justify-between sm:items-start gap-2">
                    ...
                    <div className="flex-shrink-0 space-x-1 mt-2 sm:mt-0 self-start sm:self-center">
                      <button 
                        onClick={() => handleEditMeal(entry.id)}
                        className="p-1.5 text-xs bg-sky-500 hover:bg-sky-600 text-white rounded shadow-sm"
                        title="Edit Meal"
                      >
                        ✏️ Edit
                      </button>
                      <button
                        ...
                        className="p-1.5 text-xs bg-red-500 hover:bg-red-600 text-white rounded shadow-sm"
                        title="Delete Meal"
                      >
                        🗑️ Delete
                      </button>
